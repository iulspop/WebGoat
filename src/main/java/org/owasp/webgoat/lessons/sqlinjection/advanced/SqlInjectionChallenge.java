  ...
  public AttackResult registerNewUser(
    ...
    if (attackResult == null) {
      ...
      try (Connection connection = dataSource.getConnection()) {
        String checkUserQuery = "select userid from sql_challenge_users where userid = ?"; // Use a parameterized query to prevent SQL injection
        PreparedStatement statement = connection.prepareStatement(checkUserQuery);
        statement.setString(1, username_reg); // Bind the user input
